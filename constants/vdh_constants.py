VDH_DATA_URL = 'data.virginia.gov'
VDH_APP_TOKEN = 'HgYSW46xc7HFLnFzNhhABgQ5R'

FAIRFAX = 'Fairfax'
ARLINGTON = 'Arlington'
PW = 'Prince William'

VDH_NUM_LOCALITIES = 133

# https://data.virginia.gov/Government/VDH-COVID-19-PublicUseDataset-Cases/bre9-aqqr
VDH_CASES_DATA_ENDPOINT = 'bre9-aqqr'
VDH_CASES_DATA_FRAME_TOTAL_CASES_DATA_COLUMN = 'total_cases'
VDH_CASES_DATA_FRAME_HOSPITALIZATIONS_DATA_COLUMN = 'hospitalizations'
VDH_CASES_DATA_FRAME_DEATHS_DATA_COLUMN = 'deaths'
VDH_CASES_HEALTH_DISTRICT_DATA_COLUMN = 'vdh_health_district'

NUMBER_OF_DAYS_TO_INCLUDE_IN_TESTING_DATA = 7
# https://data.virginia.gov/Government/VDH-COVID-19-PublicUseDataset-Tests_by-LabReportDa/3u5k-c2gr
VDH_TESTING_BY_LAB_REPORT_DATA_ENDPOINT = '3u5k-c2gr'
VDH_LAB_REPORT_NUMBER_OF_PCR_TESTING_DATA_COLUMN = 'number_of_pcr_testing'
VDH_LAB_REPORT_NUMBER_OF_POSITIVE_PCR_TESTING_DATA_COLUMN = 'number_of_positive_pcr_testing'
VDH_LAB_REPORT_HEALTH_DISTRICT_DATA_COLUMN = 'health_district'
